...
namespace ImageNexus.BenScharbach.TWTools.SpeedCollectionComponent
{
    ...
    /// <summary>
    /// Generic <see cref="SpeedCollection{TValue}"/>, which implements the <see cref="IDictionary"/> interface, but
    /// stores values internally in a simple array.  This should be used in time-sensitive code,
    /// where performance is critical!
    /// </summary>
    /// <remarks>
    /// This should not be used for very large arrays, since the <see cref="SpeedCollection{TValue}"/> sets the requested
    ...
    public class SpeedCollection<TValue> : IDictionary<int, TValue> where TValue : class
    {
        // internal array of itemValues
        private TValue[] _itemValues;
        ...
        private readonly List<KeyValuePair<int, TValue>> _items = new List<KeyValuePair<int, TValue>>();
...
#pragma warning restore 169
        ...
        public SpeedCollection(int arraySize)
        {
            ...
            _itemValues = new TValue[arraySize];
        ...
        IEnumerator<KeyValuePair<int, TValue>> IEnumerable<KeyValuePair<int, TValue>>.GetEnumerator()
        {
            ...
            lock (ThreadLock)
            {
                ...
                for (var i = 0; i < length; i++)
                {
                    ...
                    var item = new KeyValuePair<int, TValue>(i, _itemValues[i]);
