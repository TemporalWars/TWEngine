...
namespace XnaContentZipper
{
    class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            /*var directories = new string[] {@"$(ProjectDir)ContentAlleyPack\*.xnb"};
            const string outputFileName = @"$(ProjectDir)ContentAlleyPack\Content.zip";
            const string constantsFileName = @"$(ProjectDir)ResourceId.cs";
            const string constantNameSpace = "Spacwar";
            const string constantClassName = "ResourceId";

            KeyValuePair<int, long> keyValue = ZippedContent.ZipContent(directories, outputFileName, constantsFileName, constantNameSpace, constantClassName, null);*/
            return 0;
        }
    }
}
