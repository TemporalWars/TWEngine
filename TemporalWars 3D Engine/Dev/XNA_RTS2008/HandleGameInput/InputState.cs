...
namespace ImageNexus.BenScharbach.TWEngine.HandleGameInput
{
    ...
    public class InputState
    {
        ...
        /// on either keyboard or gamepad.
        /// </summary>
        public void ShowMarketplaceGuide(SignedInGamer signedInGamer)
        {
            try
            {
                ...
                if (!signedInGamer.Privileges.AllowPurchaseContent) return;

                // 10/5/2012 - Check if playerIndex differs from controllerIndex
                if (signedInGamer.PlayerIndex != _currentPlayerIndex)
                {
                    var buttons = new List<string>(2){ "Ok", "Cancel"};
                    var messageToDisplay1 = string.Format("Xbox requires the current gamer's controller be a LIVE signed-in profile.\n ");
                    var messageToDisplay2 =
                        string.Format(
                            "You are using controller {0}, but your LIVE signed-in profile is tied to controller {1}.\n ",
                            _currentPlayerIndex, signedInGamer.PlayerIndex);
                    const string messageToDisplay3 = "Choose the proper LIVE signed-in profile from the following signed-in guide.";
                    var messageToDisplay = string.Concat(messageToDisplay1, "\n", messageToDisplay2, "\n", messageToDisplay3);

                    Guide.BeginShowMessageBox(_currentPlayerIndex, "Live Controller Required", messageToDisplay,
                                              buttons, 0, MessageBoxIcon.Warning, ShowMessageBoxCallback, null);
                ...
                if (!Guide.IsVisible)
                    Guide.ShowMarketplace(_currentPlayerIndex); // Removed - _signedInGamer.PlayerIndex
            }
        ...
        // 10/5/2012
        private void ShowMessageBoxCallback(IAsyncResult ar)
        {
            int? buttonPressed = Guide.EndShowMessageBox(ar);
            if (!buttonPressed.HasValue) return;
            
            // User selects 'Cancel' button
            if (buttonPressed.Value == 1) return;

            // block until guide ready
            while (Guide.IsVisible)
            {
                Thread.Sleep(250);
            }

            // otherwise user selected 'Ok', so show SignIn guide.
        ...
        public bool IsKeyPress(Keys key)
        {
            ...
            for (var i = 0; i < maxInputs; i++)
            {
                if (!IsKeyPress(key, (PlayerIndex) i)) continue;

                _currentPlayerIndex = (PlayerIndex)i; // 10/5/2012
                return true;
            }
        ...
        public bool IsNewKeyPress(Keys key)
        {
            ...
            for (var i = 0; i < maxInputs; i++)
            {
                if (!IsNewKeyPress(key, (PlayerIndex) i)) continue;
        ...
        public bool IsNewButtonPress(Buttons button)
        {
            ...
            for (var i = 0; i < maxInputs; i++)
            {
                if (!IsNewButtonPress(button, (PlayerIndex) i)) continue;
        ...
        public bool IsNewButtonReleased(Buttons button)
        {
            ...
            for (var i = 0; i < maxInputs; i++)
            {
                if (!IsNewButtonReleased(button, (PlayerIndex) i)) continue;
        ...
        public bool IsOldButtonPress(Buttons button)
        {
            ...
            for (var i = 0; i < maxInputs; i++)
            {
                if (!IsOldButtonPress(button, (PlayerIndex) i)) continue;
        ...
        public bool IsButtonPress(Buttons button)
        {
            ...
            for (var i = 0; i < maxInputs; i++)
            {
                if (!IsButtonPress(button, (PlayerIndex) i)) continue;
